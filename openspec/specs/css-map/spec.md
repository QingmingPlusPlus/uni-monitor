...
- **THEN** 信息区靠设备左侧显示
- **AND** 右侧剩余区域保持白色留白，有可见变化点时允许 5M 行类别色背景延伸覆盖该行留白
...
### Requirement: 进行中变化点整行高亮

当开启变化点显示且设备存在可见变化点时，Sprite 与 CSS3D 回退必须（MUST）高亮 5M 行。每条可见记录独立占一个色块，使用自身菱形类别色的 28% 不透明度作为背景，图标必须（MUST）位于该色块中心；背景和图标共用槽位，换行时一起换行，重复类别不得合并。单一类别整行使用类别底色，多类别整行使用中性底色；标签、空余区域和超量 `+N` 不得错误表达另一类别。不得按去重类别独立均分整行背景。高亮不表达设备故障，不使用闪烁动画。

5M 图标必须（MUST）在可用空间内尽可能放大：横向最大 42px、纵向最大 32px，尺寸必须受当前行宽高和槽位数约束。信息宽度估算必须同步为较大图标预留空间，方向和档位仍不得因缩放改变；保留现有类别色、排列顺序和容量溢出汇总，不改变设备占位和点击区域。

#### Scenario: 多类别与重复记录

- **WHEN** 同一设备具有多种类别变化点或同类多条变化点
- **THEN** 每条可见记录具有独立的同类别背景槽位，图标居中；纵向换行和重复类别均保持一一对应
- **AND** 同类记录数不改变色段宽度，单一类别整行使用同一底色
- **AND** 图标保持原有次序，图标与文字在背景上可辨认，`+N` 使用中性色块

#### Scenario: 高亮覆盖右侧留白

- **WHEN** 横向或纵向设备具有可见变化点
- **THEN** 背景包含 5M 标签和标记，并延伸至设备内侧右边框，覆盖该行右侧留白
- **AND** 人员行、设备状态区、负荷率和外框保持原有颜色
- **AND** 多标记、`+N` 汇总及地图缩放不改变高亮条件

#### Scenario: 多边形与右侧 L 型设备

- **WHEN** 有变化点的设备使用多边形轮廓或有效右侧 L 型专用布局
- **THEN** 高亮按设备实际轮廓裁剪
- **AND** 右侧 L 型只高亮顶部横条内的 5M 行，到右侧竖条起点结束，凸出竖条继续留白

#### Scenario: 关闭显示或记录消失

- **WHEN** 关闭变化点显示，或设备无可见变化点，或刷新后变化点清空
- **THEN** 5M 行不显示类别色背景
- **AND** 后续刷新出现可见变化点且开启显示时恢复高亮
...
### Requirement: 地图只展示进行中的设备变化点

地图必须（MUST）通过 `GET /schedule/getChangePoint` 省略可选的 `dept/process` 参数查询全部范围，保留 `progress=`，再按 `deviceCode` 关联设备。`device` 为设备名称，不得用来匹配设备编码；`deviceCode` 缺失、为空或无法匹配时不挂载到设备。仅 `status` 为数值 `0` 或字符串 `"0"` 的记录可显示；缺失、空值和其他状态不得（MUST NOT）显示。此约定以 2026-09-18 用户确认优先于旧 Swagger。

#### Scenario: 混合状态与工序

- **WHEN** 一次查询返回不同工序设备的进行中、已关闭和无状态记录
- **THEN** 每台设备只显示自身 `status=0` 的记录
- **AND** 非设备关联记录不挂载到地图设备

#### Scenario: 查询失败

- **WHEN** 全部变化点请求发生 HTTP 或业务失败
- **THEN** 地图记录告警并显示空变化点集合，设备其他实时数据仍可加载
