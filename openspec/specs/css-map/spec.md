...
当开启变化点显示且设备存在可见变化点时，Sprite 与 CSS3D 回退必须（MUST）使用菱形类别色的 28% 不透明度高亮 5M 整行。单一类别铺满整行；多个类别按设备记录首次出现顺序去重并从左至右等宽硬分段，包含 `+N` 中的类别，不重排原有图标。背景必须（MUST）位于标签和标记下方，独立于标记排列范围；高亮表达变化点类别，不表达设备故障，不使用闪烁动画。
...
- **THEN** 各类别只出现一段同色系背景，所有色段等宽且具有清晰边界
- **AND** 同类记录数不改变色段宽度，单一类别整行使用同一底色
- **AND** 图标保持原有次序，图标与文字在背景上可辨认
...
地图必须（MUST）通过 `GET /schedule/getChangePoint` 省略可选的 `dept/process` 参数查询全部范围，保留 `progress=`，再按 `device` 关联设备。仅 `status` 为数值 `0` 或字符串 `"0"` 的记录可显示；缺失、空值和其他状态不得（MUST NOT）显示。此约定以 2026-09-18 用户确认优先于旧 Swagger。
