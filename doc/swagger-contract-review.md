...
| GET | `/schedule/getChangePoint` | 获取人机料法环数据 | `dept`*、`process`*、`progress`* |
...
- `getChangePoint`：旧文档“无查询参数”已失效；`dept/process/progress` 均必填。字段及图表分析见 [变化点统计分析](change-point-statistics.md)。
...
- `day/report`：“日报设备可动率”，响应含设备、部门、工序、`day`、`totalRunHours`、`productionHours`、`obstructionHours`、`availabilityRate` 和 `obstructionItems[]`（`pauseType/pauseTypeName/obstructionHours/count/ratio`）。未说明比率尺度、时间扣除和生产线重叠去重规则，尚不能直接满足项目日报契约。
...
本次仅更新文档及契约快照，未修改访问层、地图、卡片或既有 spec。认证失败不能解释为数据为空；旧文档中的历史实测结果不代表本次已重验。
