...
地图渲染补充规则：带 `children` 的外层设备组不显示，每个子设备直接使用自身数据独立显示；子设备的 `x`、`y`、`width`、`height` 按外层设备内部 100×100 局部坐标换算到地图坐标，以自身实际位置和尺寸渲染。无 `children` 的单设备保持原布局。设备内部信息区按原始宽高选择纵向或横向结构，并根据名称、工况、负荷率、可见人员和可见 5M 数量估算所需宽度后进入稳定档位；设备占位宽于内容档位时，信息靠左且右侧留白，外框和点击区域仍覆盖完整设备。该方向和档位不随地图缩放变化。人员配置继续显示班次扇形标记且不展示姓名；5M 继续显示“人/机/料/法/环”标记；纵向最多六槽、横向最多五槽，超出部分以 `+N` 汇总。小设备名称可省略，放大后恢复完整显示。Sprite 与 CSS3D 回退使用同一布局规划规则。
...
显示规则：仅工序维度展示。表格保留月列，折线图不展示月列。折线图两条 y 轴均自适应数据范围，不固定上限：计划/实绩生产数轴与生产达成率轴均由 ECharts 依当前周/日数据自动计算刻度，达成率轴保留百分比标签。
