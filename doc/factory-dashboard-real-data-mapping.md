...
| 5M 变化点 | `GET /schedule/getChangePoint` | `device`、`type`、`changePointContent`、`notes` | 按可见设备所属科室/工序去重查询 `dept/process/progress=`，再按设备编码匹配；前处理1/2分开请求；`type` 映射为人、机、料、法、环。非设备关联记录仅由独立变化点统计卡计入。HTTP 或业务失败记录带范围的告警，该范围降级为空，不影响其他范围的标记。 |
