...
| 5M 变化点 | `GET /schedule/getChangePoint` | `device`、`status`、`type`、`changePointContent`、`notes` | 省略 `dept/process`，携带 `progress=` 一次查询全部；按设备编码关联，只显示 `status=0`（兼容字符串）的进行中记录，缺失或其他状态不显示。非设备记录不挂载。HTTP 或业务失败告警并降级为空，不影响设备其他实时数据。 |
...
5M 行高亮：仅当开启变化点显示且过滤后的设备变化点集合非空时，用各类别菱形填充色的 28% 不透明度铺底。类别按记录首次出现顺序去重，从左至右等宽分段（含 `+N` 中的类别）；单类铺满整行，同类多条记录不增加色段。背景包含标签并覆盖该行至设备内侧右边框的留白；多边形按轮廓裁剪，右侧 L 型止于竖条起点。Sprite 与 CSS3D 回退共用色带计划，图标保持原有排列和类别色，在空间允许时放大至横向 42px、纵向 32px，信息宽度估算同步预留。关闭显示或刷新清空集合时取消高亮。
