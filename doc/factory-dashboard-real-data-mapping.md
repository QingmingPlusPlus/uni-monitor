...
地图渲染补充规则：带 `children` 的外层设备组不显示，每个子设备直接使用自身数据独立显示；当前按外层占位的长边方向水平或垂直均分，后续可逐台调整位置。无 `children` 的单设备保持原布局。人员配置在卡片上显示班次扇形标记，不直接展示姓名；5M 在卡片上显示“人/机/料/法/环”标记；小设备名称可省略，放大后恢复完整显示。
