...
地图渲染补充规则：带 `children` 的外层设备组不显示，每个子设备直接使用自身数据独立显示；子设备的 `x`、`y`、`width`、`height` 按外层设备内部 100×100 局部坐标换算到地图坐标，以自身实际位置和尺寸渲染。无 `children` 的单设备保持原布局。人员配置在卡片上显示班次扇形标记，不直接展示姓名；5M 在卡片上显示“人/机/料/法/环”标记；小设备名称可省略，放大后恢复完整显示。
