...
| 5M 变化点 | `GET /schedule/getChangePoint` | `device`、`type`、`changePointContent`、`notes` | 按设备编码匹配；`type` 映射为人、机、料、法、环。地图仍无参数调用，需适配 `dept/process/progress`；非设备关联记录仅由独立变化点统计卡计入。 |
