...
| `src/pages/daily-report`、`reportApi.ts`、`reportAdapters.ts`、`reportSources.ts`、`src/api/dailyReport.ts`、制造日报、日报接口、检查包装工序归属 | `doc/daily-report.md`、`doc/api-module.md` | `openspec/specs/manufacturing-daily-report/spec.md` | `cross-memory-routing` | 同步日报来源、显示模型、工序归并、口径、接口缺口、测试与路由 | 生效（2026-09-20出勤/设备已恢复；小数出勤、部分记录降级、空源/全零提示已接入；出勤按参考图只显示班长与统计表，复用日报共用组件及设计变量，分类零值留白、隐藏元信息，生产仅保留汇总与纵向设备排行，删除原型注释及图外说明/展开入口，比例单位未明保留缺失；品质改为九列合并表格、隐藏红字注释及图外内容；数量和品质字段仍缺） |
| `src/api/http.ts`、`src/api/requestScheduler.ts`、请求并发/连接排队/超时、`src/api/*`、`doc/api-reference.md`、Swagger 核验、`daily-net`、`day/report`、`getShijiByDate`、5M 字段、接口域、`/api` 前缀、开发代理 | `doc/api-module.md`、`doc/api-reference.md`、`doc/factory-dashboard-real-data-mapping.md`、`doc/department-api-gaps.md` | - | - | 更新 API 模块、接口参考与实测依据、字段映射、接口缺口或代理说明 | 生效（2026-09-20：25个端点；日报实测及变化点新增参数已记录） |
| Swagger 核对、`doc/swagger/`、`getChangePoint`、变化点统计/关闭率/模具类别 | `doc/swagger-contract-review.md`、`doc/change-point-statistics.md`、`doc/api-module.md`、`doc/factory-dimensions.md` | - | `cross-memory-routing` | 同步契约快照、统计分析、字段映射与接口缺口；建议未实施前不写为生效规则 | 统计卡已接入；地图全量查询并筛选status=0，2026-09-20省略范围参数已实测，新增筛选行为待验证 |
...
| `src/components/equipment-production-plan-table`、`equipmentDetailMock.ts`、设备生产计划实绩表、计划状态展开 | `doc/factory-dimensions.md`、`doc/factory-dashboard-architecture.md` | `openspec/specs/equipment-production-plan-table/spec.md` | - | 更新设备计划字段、计算口径、展开规则、mock 边界和相关测试 | 生效 |
| `src/components/css-map`、`SpriteCssMapPanel`、`public/factory-map`、`src/static/factory-map`、`scripts/generate_factory_floorplan.py`、`scripts/generate_factory_map_layout.py`、`css-map` 厂区地图、实际比例布局、设备显示白名单、`visibleDeviceCodes`、设备卡片自适应布局、设备多边形、L 型专用内容布局、5M 整行类别色带与图标放大 | `doc/factory-dimensions.md`、`doc/factory-dashboard-real-data-mapping.md`、`doc/factory-map-layout-generation.md`、`DESIGN.md` | `openspec/specs/css-map/spec.md` | - | 更新 css-map spec、工厂维度 doc、实际比例生成 doc、接口映射 doc、设计系统或地图组件 | 生效 |
...
| `src/pages/factory-dashboard/components/PersonnelAttendanceCard`、`loadAttendanceCard`、`人员出勤情况`、人员子类/新人列 | `doc/factory-dashboard-real-data-mapping.md`、`doc/department-api-gaps.md`、`doc/factory-dimensions.md` | `openspec/specs/personnel-attendance-table/spec.md` | - | 更新人员分类、合计与列展示规则、接口映射、接口缺口和相关测试 | 生效 |
...
| `src/components/change-point-card`、`loadChangePointCard`、部门/工序变化点明细表、`endDate` 解除日期、日期/班次/状态筛选 | `doc/change-point-statistics.md`、`doc/factory-dashboard-architecture.md`、`doc/shared-utilities.md` | - | `cross-memory-routing` | 更新变化点明细、筛选、状态匹配、接口缺口与装配文档 | 生效 |
