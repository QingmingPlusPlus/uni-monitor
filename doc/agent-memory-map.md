...
| `src/pages/daily-report`、`reportApi.ts`、`reportAdapters.ts`、`reportSources.ts`、`src/api/dailyReport.ts`、制造日报、日报接口、检查包装工序归属 | `doc/daily-report.md`、`doc/api-module.md` | `openspec/specs/manufacturing-daily-report/spec.md` | `cross-memory-routing` | 同步日报来源、显示模型、工序归并、口径、接口缺口、测试与路由 | 生效（2026-09-20出勤/设备已恢复；小数出勤、部分记录降级、空源/全零提示已接入；出勤按参考图只显示班长与统计表，复用日报共用组件及设计变量，分类零值留白、隐藏元信息，数量和品质字段仍缺） |
