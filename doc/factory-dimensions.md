...
- 页面布局：顶部为 `Omni Monitor` + 当前工序 Mock 告警跑马灯；主体仿照部门维度，左侧三分之一为 `css-map`，右侧三分之二直接进入可滚动瀑布流（不再使用 1:1 等宽双栏，也不展示瀑布流上方 KPI 网格）。瀑布流第一项是跨整行的人员出勤情况（mock）卡片，后续按以下顺序展示：不良率计画实绩（金额）、不良率计画实绩（个数）、MH 实绩、出勤率推移表（mock）、入库计划推移表（mock）、人员明细及状态（mock）、生产计划实绩推移表（mock，行简化为计划/实绩两行）。
...
| `pages/process/index` | 工序 | `processId` | 告警栏 + 左三分之一 `css-map` + 右侧单/双列多列瀑布流（部门维度所有组件 + 生产计划实绩推移，1 课工序隐藏入库计划） |
