...
- 右侧 L 型设备可额外配置 `contentLayout: "right-l-shape"`。渲染器会从 `polygon` 提取横条高度和右侧竖条起点：名称与状态位于顶部横条，人员与 5M 位于横条下半区，负荷率独立位于右侧竖条，确保原有五类内容都落在多边形有效区域内。当前 `CG涂装-2` 使用该布局，右侧竖条宽度为设备宽度的 20%；其横条与 `CG涂装-4` 之间保留 10 个局部单位的纵向间隙，竖条与 `CG涂装-4` 之间保留 5 个局部单位的横向间隙。
