...
最新契约快照见 [2026-09-16 Swagger 核对](swagger-contract-review.md)，其中接入状态以当前代码及本文为准。变化点统计卡通过 `dept/process/progress` 查询，保留非设备关联记录；地图按可见设备所属科室/工序去重查询，显式传入 `progress=`，再按设备编码关联。API 参数为必填，避免遗漏工序触发后端空指针。统计口径见 [变化点统计](change-point-statistics.md)。
