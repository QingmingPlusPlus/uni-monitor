...
- 9月14日的考勤、基础配置和设备查询超时属于历史记录；9月20日已重验两日出勤和设备day/report，其余未重测端点保留历史依据。
...
无新增后端接口。生产汇总和产线排行均读取getPlan/getOutput/getRejects，进行中请求共享保持不变。生产专用聚合按制番或设备分组，实绩包含全部废弃、合格包含明确非不良、流动直接取getOutput；不良成功且范围完整时无记录按0。品质区与生产共用productionQuantities，继承空不良按0规则；getDailyQuality同时共享设备日报补充产线名称，失败回退编码，不影响有效数量。设备即产线，availabilityRate/ratio按原数值展示，计划停止列暂不显示。详细公式与边界见[制造日报](daily-report.md)。
