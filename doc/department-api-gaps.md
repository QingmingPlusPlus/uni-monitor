...
本节取代下方与日报有关的超时结论；其他未重测接口仍保留历史状态。完整请求见 [接口参考](api-reference.md)，逐项补数清单见 [制造日报](daily-report.md)。

- 两日出勤已恢复，两个查询各3行；分类存在0.5/1.5等小数，前端已修复被整数校验隐藏的问题，保留小数及差异提示。后端需核对在籍0/实绩29和分类合计不等于缺勤总数的情况，并明确折算单位、班次状态及时间。
- 设备day/report已恢复，三个查询35/59/56台，但时间、可动率全0、阻碍原因全空。页面如实展示并提示核对采集完整性；比例单位/分母、非零阻碍项仍未确认。daily-net等其他设备端点本次未重测。
- 生产计划9月5775条、7月1105条；实绩9月0条、7月9条、5月2条；不良5/7/9月均空。合格数、流动数、模具关联、不良现象和权威生产线归属仍缺失。
- getChangePoint仅传progress=成功返回2条，包含banci/endDate/status；Swagger新增日期及班次参数，但仍将实际可省略的范围参数标为必填。新筛选行为未验证。

## 2026-09-14 核验与定义更新（历史）
