...
describe('loadAttendanceCard', () => {
  ...
  it('多工序族时部门全体合计行汇总所有明细，不返回 0', async () => {
            ...
            data: [
              {
                ...
                positionName: '正式工',
                schedulePersonCount: 8,
                actualAttendancePersonCount: 6,
                positionType: 'direct',
              },
              {
                shiftType: 'day',
                shiftTypeName: '早班',
                positionId: 'newcomer-day',
                positionName: '新人',
                schedulePersonCount: 2,
                actualAttendancePersonCount: 2,
                positionType: 'direct',
              },
              {
                ...
                positionId: 'regular',
                positionName: '正式工',
                schedulePersonCount: 19,
                actualAttendancePersonCount: 17,
                positionType: 'direct',
              },
              {
                shiftType: 'middle',
                shiftTypeName: '中班',
                positionId: 'newcomer-middle',
                positionName: '新人',
                schedulePersonCount: 1,
                actualAttendancePersonCount: 1,
                positionType: 'direct',
        ...
        data: {
          ...
          data: [
            ...
            },
            {
              ...
              positionId: 'regular',
              positionName: '正式工',
              schedulePersonCount: 13,
              actualAttendancePersonCount: 10,
              positionType: 'direct',
            },
            {
              shiftType: 'night',
              shiftTypeName: '晚班',
              positionId: 'newcomer-night',
              positionName: '新人',
              schedulePersonCount: 2,
              actualAttendancePersonCount: 2,
              positionType: 'direct',
    ...
    expect(allGroup?.rows.find((row) => row.shift === 'day')).toMatchObject({
      directRegular: 12,
      directNewcomer: 3,
      directRosterTotal: 15,
      actualAttendance: 12,
...
describe('createProductionPlanTrendCards', () => {
  beforeEach(() => {
    vi.useFakeTimers()
  ...
  it('按工序族生成六行名称，部门多工序保持配置顺序', () => {
    const pretreatment = createProductionPlanTrendCard('department1', 'pretreatment1')
    const vulcanization = createProductionPlanTrendCard('department2', 'vulcanization1')
    const posttreatment = createProductionPlanTrendCard('department3', 'posttreatment1')
    const departmentCards = createProductionPlanTrendCards(
      'department4',
      ['vulcanization2', 'posttreatment2'],
    )

    expect(pretreatment?.tableRows.map((row) => row.label)).toEqual([
      '计划粘接数',
      '实绩粘接数',
      '计划MH',
      '实绩MH',
    ...
    expect(vulcanization?.tableRows.slice(0, 2).map((row) => row.label)).toEqual([
      '计划加硫数',
      '实绩加硫数',
    ])
    expect(posttreatment?.tableRows.slice(0, 2).map((row) => row.label)).toEqual([
      '计划入库数（含待倒箱、端数等）',
      '实绩入库数（含待倒箱、端数等）',
    ])
    expect(departmentCards.map((card) => card.id)).toEqual([
      'production-plan-trend:vulcanization2',
