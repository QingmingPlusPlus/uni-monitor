...
watch(() => props.scope, () => { expanded.value = false })
onBeforeUnmount(() => {
...
<template>
  <view class="change-point-card">
    <view class="change-point-card__header">
      <view>
        <text class="change-point-card__title">变化点统计</text>
        <text class="change-point-card__subtitle">{{ scope }} · {{ data.month }} · 本周 {{ data.weekDays[0] }}–{{ data.weekDays[data.weekDays.length - 1] }} 日</text>
      </view>
      <view class="change-point-card__actions">
        <button :disabled="data.status === 'loading'" aria-label="刷新变化点统计" @click="emit('refresh')"><svg class="change-point-card__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M17.65 6.35A7.95 7.95 0 0 0 12 4a8 8 0 1 0 7.45 5.08h-2.13A6 6 0 1 1 12 6a5.96 5.96 0 0 1 4.24 1.76L13 11h8V3l-3.35 3.35Z" /></svg></button>
        <button aria-label="展开变化点统计" @click="expanded = true"><svg class="change-point-card__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 5h6v2H8.41l3.3 3.29-1.42 1.42L7 8.41V11H5V5Zm8 0h6v6h-2V8.41l-3.29 3.3-1.42-1.42L15.59 7H13V5Zm4 10.59V13h2v6h-6v-2h2.59l-3.3-3.29 1.42-1.42L17 15.59ZM8.41 17H11v2H5v-6h2v2.59l3.29-3.3 1.42 1.42L8.41 17Z" /></svg></button>
      </view>
    </view>
    ...
    <ChangePointContent v-if="data.status !== 'loading'" :data="data" />
    <view v-if="expanded" class="change-point-card__overlay" role="dialog" aria-modal="true" aria-label="变化点统计全月详情" @click.self="expanded = false">
      <view class="change-point-card__modal">
        <view class="change-point-card__header">
          <view><text class="change-point-card__title">变化点统计</text><text class="change-point-card__subtitle">{{ scope }} · {{ data.month }}</text></view>
          <button aria-label="关闭变化点统计" @click="expanded = false"><svg class="change-point-card__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 16h3v3h2v-5H5v2Zm3-8H5v2h5V5H8v3Zm6 11h2v-3h3v-2h-5v5Zm2-11V5h-2v5h5V8h-3Z" /></svg></button>
        </view>
        <view v-if="data.message" class="change-point-card__message" role="status">{{ data.message }}</view>
        <view v-if="data.status === 'loading'" class="change-point-card__loading"><LoadingIcon /></view>
        <ChangePointContent v-else :data="data" expanded />
      </view>
    </view>
  </view>
</template>
